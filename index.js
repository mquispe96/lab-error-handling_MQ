...
function filterProductsByPriceRange(products, min, max) {
	...
	if(typeof min !== 'number' || typeof max !== 'number'){
		throw 'Min or max need to be a number';
	}
	if(!max){
		throw 'Max cant be zero';
	}
	if(min > max){
		throw 'Min cant be greater than max';
	}
	if(min < 0 || max < 0){
		throw 'Min and max need to be greater than zero';
	}
	if(!products.every(product => product.priceInCents)){
		throw 'Every product needs a priceInCents key';
	} 
  const result = [];
